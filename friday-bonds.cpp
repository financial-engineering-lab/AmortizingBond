...
int main(int, char* []) {
    ...
    try {
        ...
        * issueDate - The issue date is the date on which
        *  bond begins to accrue interest. Maturity date
        *  is when the bond matures and the bond issuer
        *  must pay the principal amount to bondholders.
        * effectiveDate - the date that an agreement or
        *  transaction between or among signatories becomes binding.
...
Date issueDate =todayDate;
std::cout <<  "issuDate = " << issueDate <<  endl;

Period numberPeriod = Period(1,Years);
std::cout << "number of periods = " << numberPeriod <<  endl;

/*maturityDate - the date the contract is inactive*/

Date maturityDate=issueDate+numberPeriod;
std::cout << "maturityDate = " << maturityDate <<  endl;


                Schedule amortizingBondSchdule(issueDate,                         //const Date& effectiveDate
        ...
        std::vector<double> discountFactorFRA;
        std::vector<double> analyticalDiscountFactor;
        std::vector<double> compoundFactor;
        std::vector<double> compoundFactorFRA;
        std::vector<double> analyticalCompoundFactor;
        std::vector<double> cumulativeInterest;
        std::vector<double> CumulativeInterestAnal;
        std::vector<double> interest;
        std::vector<double> interestAnal;
        std::vector<double> interestFRA;
        std::vector<double> CumulativePrincipalPaid;
        std::vector<double> CumulativePrincipalPaidAnal;
        std::vector<double> CumulativePrincipalPaidFRA;
        std::vector<double> principalPaid;
        std::vector<double> PrincipalPaidAnal;
        std::vector<double> PrincipalPaidFRA;
        std::vector<double> loanBalance;
        std::vector<double> LoanBalanceAnal;
        std::vector<double> LoanBalanceFRA;
        
  
        double compoundFactorSum=0;
        double compoundFactorSumAnal=0;
        double compoundFactorSumFRA=0;
        double normalizedAmortizingCoupon;
        double normalizedAmortizingCouponAnal;
        double normalizedAmortizingCouponFRA;
        double  compoundFactor_j = 1;
        ...
        //For a general discounting series to work we need a set of forward rates 
        //do discount the j coupon
        //***Inthe case of constant interest rate the compunding should be equivelent to 
        //Thirty360 day convention how ever it looks like this is not the case:(((
 
 
        //=======================================
        //     ****compoundFactor computation
        //=======================================
 
        t=double(amortizingBondSchdule[j]-issueDate)/365; //need to compare*t = dc_.yearFraction(d1, d2, refStart, refEnd)
        Rate compoundFactorR =interest_rate.compoundFactor(t);
        compoundFactor.push_back(compoundFactorR);        
        
        //=======================================
        //     ****analytical compoundFactor computation
        //=======================================
        
        Time t_anl=1/12*j;
        double j_d=double(j);
        Rate CompoundFactorAnal=pow((1+double(rate/12)),j_d);   
        analyticalCompoundFactor.push_back(CompoundFactorAnal);
        
        //=======================================
        //     ****compoundFactorFRA computation
        //=======================================
        ...
        if(j==0){
                compoundFactorFRA.push_back(compoundFactor_j);
        }

        else {
        Time t_maturity= double(maturityDate-issueDate)/365;
        Time t_fra=double(amortizingBondSchdule[j]-issueDate)/365;

        //need to compare*t = dc_.yearFraction(d1, d2, refStart, refEnd)
        double compundFactorMatueity =interest_rate.compoundFactor(t_maturity);
        double compoundFactor_j =interest_rate.compoundFactor(t_fra);
        double compoundFactor_j_maturity = compundFactorMatueity/compoundFactor_j;
        //compoundFactor_j = compoundFactor_j*compoundFactor_temp;
        //compoundFactor_temp=compoundFactor_j;
        compoundFactorFRA.push_back(compoundFactor_j_maturity);          
        }
         
 
        if(j<int(amortizingBondSchdule.size()-1)) {
                compoundFactorSum += compoundFactor[j];
                compoundFactorSumAnal += analyticalCompoundFactor[j];
                compoundFactorSumFRA += compoundFactorFRA[j];       
        }       
        std::cout << amortizingBondSchdule[j] <<"---"<< compoundFactorR <<"---"<< compoundFactor[j] <<"---"<< compoundFactor[j]<<"---"<< t << endl;
        std::cout << amortizingBondSchdule[j] <<"---"<< CompoundFactorAnal <<"---"<< analyticalCompoundFactor[j] <<"---"<< compoundFactor[j]-analyticalCompoundFactor[j]<<"---"<< t_anl << endl;
        std::cout << amortizingBondSchdule[j] <<"---"<< compoundFactor_j <<"---"<< compoundFactorFRA[j] <<"---"<< analyticalCompoundFactor[j]-compoundFactorFRA[j]<<"---"<< t_fra << endl;
        }
        


        normalizedAmortizingCoupon=interest_rate.compoundFactor(t)/compoundFactorSum;
        normalizedAmortizingCouponAnal=interest_rate.compoundFactor(t)/compoundFactorSumAnal;
        normalizedAmortizingCouponFRA = interest_rate.compoundFactor(t)/compoundFactorSumFRA;

        std::cout << normalizedAmortizingCoupon << std::endl;
        std::cout << normalizedAmortizingCouponAnal << std::endl;
        std::cout << normalizedAmortizingCouponFRA << std::endl;
 
        double amortizingCoupon = notional*normalizedAmortizingCoupon;
        std::cout << amortizingCoupon << std::endl;
        double amortizingCouponAnal = notional*normalizedAmortizingCouponAnal;
        std::cout << amortizingCouponAnal << std::endl;
        double amortizingCouponFRA = notional*normalizedAmortizingCouponFRA;
        std::cout << amortizingCouponFRA << std::endl;
        ...
        loanBalance.push_back(notional);
        principalPaid.push_back(0);
        interest.push_back(0);
        cumulativeInterest.push_back(0);
        double cumulativeInterest_temp=0;

        for(int j=0; j < int(amortizingBondSchdule.size()-1);++j){
        t=double(amortizingBondSchdule[j+1]-amortizingBondSchdule[j])/365;
        double a =interest_rate.compoundFactor(t);
        double b = loanBalance[j]*a-amortizingCoupon;   
        loanBalance.push_back(b);
        double pB=loanBalance[j]-loanBalance[j+1];
        principalPaid.push_back(pB);
        double interest_temp=amortizingCoupon-principalPaid[j+1];
        interest.push_back(interest_temp);        
        cumulativeInterest_temp += interest[j+1];
        cumulativeInterest.push_back(cumulativeInterest_temp);
        std::cout << amortizingBondSchdule[j+1] <<"---"<< loanBalance[j+1] <<"---"<< principalPaid[j+1] << "---" << interest[j+1] << "---"<<  "---"<< cumulativeInterest[j+1]<< endl;
        }
  

        //==============================================================
        //                     Loan balance  Analitic Computation
        //==============================================================

        LoanBalanceAnal.push_back(notional);
        PrincipalPaidAnal.push_back(0);
        interestAnal.push_back(0);
        CumulativePrincipalPaidAnal.push_back(0);
        cumulativeInterest_temp=0;


        for (int j=0; j < int(amortizingBondSchdule.size())-1;++j){
        //t=double(amortizingBondSchdule[j+1]-amortizingBondSchdule[j])/365;
        double a =pow((1+double(rate/12)),1);  
        double b = LoanBalanceAnal[j]*a-amortizingCouponAnal;     
        LoanBalanceAnal.push_back(b);
        double principalPaid_temp=LoanBalanceAnal[j]-LoanBalanceAnal[j+1];
        PrincipalPaidAnal.push_back(principalPaid_temp);
        double interest_temp=amortizingCouponAnal-PrincipalPaidAnal[j+1];
        interestAnal.push_back(interest_temp);        
        cumulativeInterest_temp += interestAnal[j+1];
        CumulativePrincipalPaidAnal.push_back(cumulativeInterest_temp);
        std::cout << amortizingBondSchdule[j+1] <<"---"<< LoanBalanceAnal[j+1] <<"---"<< PrincipalPaidAnal[j+1] << "---" << interestAnal[j+1] << "---"<< CumulativePrincipalPaidAnal[j+1]<< endl;
        
        }
        //==============================================================
        //                     FRA Loan balance Computation
        //==============================================================


        LoanBalanceFRA.push_back(notional);
        PrincipalPaidFRA.push_back(0);
        interestFRA.push_back(0);
        CumulativePrincipalPaidFRA.push_back(0);
        cumulativeInterest_temp=0;
        ...
        //double c =compoundFactor[j+1];
        double b = LoanBalanceFRA[j]*a-amortizingCouponFRA;   
        LoanBalanceFRA.push_back(b);
         double principalPaid_temp=LoanBalanceFRA[j]-LoanBalanceFRA[j+1];
        PrincipalPaidAnal.push_back(principalPaid_temp);
        double interest_temp=amortizingCouponAnal-PrincipalPaidAnal[j+1];
        interestFRA.push_back(interest_temp);        
        cumulativeInterest_temp += interestFRA[j+1];
        CumulativePrincipalPaidFRA.push_back(cumulativeInterest_temp);
        std::cout << amortizingBondSchdule[j+1] <<"---"<< LoanBalanceFRA[j+1]
         <<"---"<< PrincipalPaidAnal[j+1] << "---" << interestFRA[j+1]
          << "---"<< CumulativePrincipalPaidFRA[j+1]<< endl;
       }


        /*CumulativePrincipalPaidFRA
